...
<body class="min-h-screen antialiased">
    ...
    <header class="border-b border-ink-line">
        <div class="mx-auto flex max-w-6xl flex-wrap items-center gap-6 px-6 py-4">
            ...
            @auth
                ...
                <form method="POST" action="{{ route('panel.logout') }}" class="ml-auto">
                    @csrf
    ...
    <main class="mx-auto max-w-6xl px-6 py-10">
        ...
        @if (session('error'))
            <div class="mb-6 rounded-lg border border-amber-500/40 bg-amber-500/10 px-5 py-3 text-sm text-amber-200">
                {{ session('error') }}
            </div>
        @endif
